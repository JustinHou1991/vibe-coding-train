class LunarCalendar {
    ...
    solar2lunar(y, m, d) {
        ...
        return {
            ...
            monthCn: (isLeap ? '闰' : '') + this.lunarMonths[month - 1] + '月',
            dayCn: this.lunarDays[day - 1],
            animal: this.animals[(year - 4) % 12],
            fullString: '农历' + this.getYearCn(year) + '年' + (isLeap ? '闰' : '') + this.lunarMonths[month - 1] + '月' + this.lunarDays[day - 1]
        };
    }
    ...
    formatLunarDate(dateString) {
        const lunar = this.getLunarDate(dateString);
        if (!lunar) return '';
        return lunar.fullString;
    }
